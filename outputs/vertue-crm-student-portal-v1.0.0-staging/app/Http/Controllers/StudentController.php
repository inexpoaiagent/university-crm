...
class StudentController extends Controller
{
    ...
    public function show(Request $request, int $id): View
    {
        ...
        $docs = Document::query()->forTenant($user->tenant_id, $user->role_slug)->where('student_id', $student->id)->latest('id')->get();
    ...
    public function uploadDocument(Request $request, int $id): RedirectResponse
    {
        ...
        $this->audit($request, 'student.document.upload', 'student', $student->id, ['type' => $data['type']]);

        return back()->with('success', 'Document uploaded for student.');
    }

    public function verifyDocument(Request $request, int $id, int $docId): RedirectResponse
    {
        $user = $this->authUser($request);
        Student::query()->forTenant($user->tenant_id, $user->role_slug)->whereNull('deleted_at')->findOrFail($id);
        $document = Document::query()
            ->forTenant($user->tenant_id, $user->role_slug)
            ->where('student_id', $id)
            ->findOrFail($docId);
        $document->update(['status' => 'verified']);
        $this->audit($request, 'student.document.verify', 'document', $document->id, ['student_id' => $id]);

        return back()->with('success', 'Document verified.');
    }

    public function deleteDocument(Request $request, int $id, int $docId): RedirectResponse
    {
        $user = $this->authUser($request);
        Student::query()->forTenant($user->tenant_id, $user->role_slug)->whereNull('deleted_at')->findOrFail($id);
        $document = Document::query()
        ...
        $storedPath = ltrim(str_replace('/storage/', '', (string) $document->file_url), '/');
        if ($storedPath !== '' && Storage::disk('public')->exists($storedPath)) {
            Storage::disk('public')->delete($storedPath);
        }
        $document->delete();
        $this->audit($request, 'student.document.delete', 'document', $docId, ['student_id' => $id]);
